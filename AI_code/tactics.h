...
/**
 * @brief The externalRobot struct
 *
 * contains information about other robots
 */
...
/**
 * @brief The RobotMap class
 *
 *  map of the robot used for mapping
 */
class RobotMap{
public:
    ...
    void setValue(int GPSx,int GPSy,double value){
    ...
    double getValue(int GPSx,int GPSy){
        int valx = GPSx-offsetX+25;
        valx = (valx < 0) ? 0 : (valx > 50) ? 50 : valx;
        int valy = GPSy-offsetY+25;
        valy = (valy < 0) ? 0 : (valy > 50) ? 50 : valy;
    ...
    double getArrayValue(int x,int y){
        return values[y][x];
    }
