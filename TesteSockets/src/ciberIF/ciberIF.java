...
try{
	    ...
	    if(qName.equals("Measures")) {         // Measures Values
		      ...
		      String idStr=attrs.getValue("Id");
	              if(idStr!=null) {
			       ...
			       values.IRSensorReady[IRid] = true;
		               values.IRSensor[IRid] = format.parse(IRValStr).doubleValue();
		           }
		      }
                 }
	    }
		           ...
		           String valStr=attrs.getValue("Value");
			        ...
			        values.beaconReady[id]=true;
		                if(!valStr.equals("NotVisible")) {
		                     values.beacon[id].beaconDir = format.parse(valStr).doubleValue();
		      ...
		      String xStr=attrs.getValue("X");             
	              if(xStr!=null) {
			    values.gpsData.x = format.parse(xStr).doubleValue();
                            values.gpsReady = true;
		      }

		      String yStr=attrs.getValue("Y");             
	              if(yStr!=null) {
			    values.gpsData.y = format.parse(yStr).doubleValue();
		      }

		      String dirStr=attrs.getValue("Dir");             
	              if(dirStr!=null) {
			    values.gpsData.dir = format.parse(dirStr).doubleValue();
		      }
...
public class ciberIF {
    ...
    */
     ...
     */
    public boolean InitRobot(String name, int pos, String hostname,int type)
    {
        //// Send init message
	// Construct message
