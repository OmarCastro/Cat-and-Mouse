...
void CRQScene::drawLab( CRLab * l_b )
{
    ...
    for ( target = tg->begin() ; target != tg->end(); ++target )
    {
        ...
        QGraphicsEllipseItem *labArrival;
        QGraphicsEllipseItem *labArrival2;

        labArrival = new QGraphicsEllipseItem(target_x, target_y,diam, diam, 0, this);
        labArrival2  = new QGraphicsEllipseItem(target_x + 0.1 * zoom, target_y + 0.1 * zoom,
        ...
        if(target == tg->begin()){
            ...
            targ1->setRect(targ1->rect().translated(-target_x, -target_y));
...
int CRQScene::drawRobot( CRLab * l_b )
{
   ...
   /* ********************
    ...
    if( rob !=NULL && (rob->id() > 0) && (rob->id() < (nRobots + 1) ) ){
        ...
        //mousePixmap->setX(rob->x() * zoom - mousePixmap->pixmap().width()/2);
        //mousePixmap->setY( sizeInPixels -  rob->y() * zoom - mousePixmap->pixmap().height()/2);
        double target_x = rob->x() * zoom - radius;
        double target_y = sizeInPixels - rob->y() * zoom - radius;
        targ1->setPos(target_x,target_y);
